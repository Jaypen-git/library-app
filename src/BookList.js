...
const BookList = ({books}) => {
    ...
    return ( 
        <div>
            {books.map(book => (
                    <div key={book.id} className={book.id % 2 === 0 ? "grid grid-cols-6 justify-center items-center py-4 px-2 bg-blue-50" : "grid grid-cols-6 justify-center items-center py-4 px-2"}
                    id={`book-${book.id}`} onClick={() => {
                        ...
                        <div className="details hidden col-span-6">
                            ...
                            <p className="col-span-4 text-2xl sm:text-3xl font-medium px-1.5 m-2 sm:m-3">{ `Pages: ${book.pages}` }</p>
                            <p className="col-span-4 text-2xl sm:text-3xl font-medium px-1.5 m-2 sm:m-3">{ `Status: ${book.status}` }</p>
                        </div>
                    </div>
            ))}
        </div>
